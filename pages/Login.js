      ...
      fetch(`http://localhost:8000/email?email=${session.user.email}`)
        ...
        .then(data => {
          ...
          if(data.length === 0) {
            ...
            setMyDetails({ name: session.user.name, email: session.user.email, userID: crypto.randomUUID() })
            console.log('userID not found')
          } else {
            console.log(data[0].userID)
            // localStorage.setItem('myDetails', JSON.stringify({ name: session.user.name, email: session.user.email, userID: data[0].userID }));
            setMyDetails({ name: session.user.name, email: session.user.email, userID: data[0].userID })
          }
        })
